...
public class MOEADD<S extends Solution<?>> extends AbstractMOEAD<DoubleSolution> {
    ...
    public MOEADD(Problem<DoubleSolution> problem, int populationSize, int resultPopulationSize, int maxEvaluations, MutationOperator mutation,CrossoverOperator crossoverOperator,  FunctionType functionType, String dataDirectory, double neighborhoodSelectionProbability, int maximumNumberOfReplacedSolutions, int neighborSize, String inProcessDataPath,int run) {
        super(problem, populationSize, resultPopulationSize, maxEvaluations, crossoverOperator,mutation,  functionType, dataDirectory, neighborhoodSelectionProbability, maximumNumberOfReplacedSolutions, neighborSize, inProcessDataPath,run);
    }
    ...
    public void run() {
        ...
        // Dept. Electr. Comput. Eng., Michigan State Univ., East Lansing,MI, USA, Tech. Rep. COIN No. 2014014, 2014.
    ...
    public double calculateDistance2(Solution indiv, double[] lambda,
        ...
        for (int i = 0; i < problem.getNumberOfObjectives(); i++) {
            realB[i] = (indiv.getObjective(i) - (z_[i] + d1 * lambda[i]));     //类似PBI
        }
