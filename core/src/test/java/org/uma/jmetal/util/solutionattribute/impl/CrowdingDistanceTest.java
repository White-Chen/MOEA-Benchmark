...
public class CrowdingDistanceTest {
    ...
    public void shouldTheCrowdingDistanceOfTwoSolutionsBeInfinity() {
        ...
        assertEquals(Double.POSITIVE_INFINITY, crowdingDistance.getAttribute(solutionList.get(0)), EPSILON);
        assertEquals(Double.POSITIVE_INFINITY, crowdingDistance.getAttribute(solutionList.get(1)), EPSILON);
    }
    ...
    public void shouldTheCrowdingDistanceOfThreeSolutionsCorrectlyAssigned() {
        ...
        assertEquals(Double.POSITIVE_INFINITY, crowdingDistance.getAttribute(solutionList.get(0)), EPSILON);
        assertEquals(Double.POSITIVE_INFINITY, crowdingDistance.getAttribute(solutionList.get(1)), EPSILON);
        assertEquals(2.0, crowdingDistance.getAttribute(solutionList.get(2)), EPSILON);
    }
}