...
class EvaluationTask<S extends Solution<?>, Result> implements Callable<Object> {
    ...
    public EvaluationTask(TaggedAlgorithm<Result> algorithm, int id, Experiment<?, ?> experimentData) {
        ...
        String inProcossDirectoryName = outputDirectoryName + "/INPROCESSDATA" + id;

        File outputDirectory = new File(outputDirectoryName);
        File inProcossDirectory = new File(inProcossDirectoryName);
        ...
        if (!inProcossDirectory.exists()) {
            boolean result = new File(inProcossDirectoryName).mkdirs();
            if (result) {
                JMetalLogger.logger.info("Creating " + inProcossDirectoryName);
            } else {
                JMetalLogger.logger.severe("Creating " + inProcossDirectoryName + " failed");
            }
        }
    }
